...
chair_mean_est= np.asarray([ 0.00210511,  0.00037186, -0.01179088])
chair_std_est= 2.9956021
car_mean_est=np.asarray([ 0.00021814,  0.00639687, -0.01720772]) 
car_std_est=2.7835798
aeroplane_mean_est= np.asarray([-0.00073359,  0.00459401,  0.08839293])
aeroplane_std_est= 3.2317474

std_est=[chair_std_est, car_std_est, aeroplane_std_est]
mean_est=[chair_mean_est, car_mean_est, aeroplane_mean_est]
#cat_ids=[chair:0, car:1, aeroplane:1]
...
def encode(pc):
    ...
    tr_pc=tr_pc*std_est[cat_id] +mean_est[cat_id]

    tr_pc= torch.from_numpy(tr_pc).float()
    tr_pc= tr_pc.cuda() if args.gpu is None else tr_pc.cuda(args.gpu)

    z= model.encode(tr_pc).detach().cpu().numpy()
    
    return z[0]

def reconstruct_from_code(z):
    arr=[]
    for i in range(10):
        arr.append(z)
    ...
    z= z.cuda() if args.gpu is None else z.cuda(args.gpu)
    y, x= model.decode(z, num_points=2048)

    x[:,:,[0,1,2]]= x[:,:,[2,0,1]]
    x[:,:,0]*=-1
    
    return x[0].detach().cpu().numpy().T